...
- **Don't remove the POCASI unit guard** in `descriptor_parser._infer_unit_from_context`: the `CAS`→`h` (time) heuristic also matches weather props `POCASI*` (string values), which then raised `ValueError`. `date`/`time` elements force `unit=None` for the same reason.
